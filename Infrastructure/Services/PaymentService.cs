    ...
    : IPaymentService
{
    public async Task<ShoppingCart?> CreateOrUpdatePaymentIntent(string cartId)
    {
        ...
        var cart =
            await cartService.GetCartAsync(cartId) ?? throw new Exception("Cart unavailable");
    ...
    private async Task ValidateCartItemsInCartAsync(ShoppingCart cart)
    {
        ...
        {
            var productItem =
                await unit.Repository<Core.Entities.Product>().GetByIdAsync(item.ProductId)
    ...
    private async Task<long?> GetShippingPriceAsync(ShoppingCart cart)
    {
        ...
        {
            var deliveryMethod =
                await unit.Repository<DeliveryMethod>().GetByIdAsync((int)cart.DeliveryMethodId)
